# PART 1: Core MCP Server File

Create the complete `mcp_server.py` file with ALL 28 tools and full production code. Do not truncate.

## Requirements
- FastAPI app with CORS + MCP mounted at /mcp
- OCIAuthManager class with Instance Principal (primary) + config file fallback
- All 28 tools in one file
- Each tool returns json.dumps(data, default=str)
- Async functions
- Full error handling with HTTPException
- Logging with loguru
- Environment variable support

## The Complete mcp_server.py

Copy this entire code exactly as-is:
...
@mcp.tool()
async def list_file_systems(ctx: Context) -> str:
    """List File Storage systems."""
    try:
        fs = auth_manager.filestorage_client.list_file_systems(compartment_id=auth_manager.compartment_id).data
...
async def get_usage_summary(ctx: Context, time_start: str, time_end: str) -> str:
    ...
    try:
        ...
        return json.dumps([u.__dict__ for u in usage.items], default=str)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
if __name__ == "__main__":
    import uvicorn
    logger.info("🚀 Starting Oracle Context MCP Server with 28 tools...")
    uvicorn.run(app, host="0.0.0.0", port=8000)
```

That's the complete mcp_server.py - all 28 tools, no truncation. Create this file exactly.
